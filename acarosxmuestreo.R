...
# Metropolitano y Soberania  (Panama) por tipo de muestreo
# 13-ene-2026
...
n.id <- acaros |> group_by(id, morfoespecie, trampa) |> 
  summarise(abundancia = sum(cantidad, na.rm=TRUE))
head(n.id)

# matriz de abundancia por cumnidades
matriz.abundancia <- pivot_wider(data = n.id, id_cols = c("id","trampa"), names_from = "morfoespecie", values_from = "abundancia")
head(matriz.abundancia)
matriz.abundancia[is.na(matriz.abundancia)] <-  0
matriz.num <- matriz.abundancia[,3:ncol(matriz.abundancia)]
matriz.sitio <- unique(acaros[,c("id","Lugar","Mes","trampa")])
...
resultado_permanova_trampa <- adonis2(matriz.num ~ trampa, 
                                         data = matriz.sitio, 
                                         method = "bray", 
                                         permutations = 999)

print(resultado_permanova_trampa)

resultado_permanova_mes_trampa <- adonis2(matriz.num ~ Mes+trampa, 
                                         data = matriz.sitio, 
                                         method = "bray", 
                                         permutations = 999)

print(resultado_permanova_mes_trampa)

resultado_permanova_lugar_trampa <- adonis2(matriz.num ~ Lugar+trampa, 
                                          data = matriz.sitio, 
                                          method = "bray", 
                                          permutations = 999)

print(resultado_permanova_lugar_trampa)
...
nmds_coords$trampa <- matriz.sitio$trampa

# grafico
ggplot(nmds_coords, aes(x = NMDS1, y = NMDS2, color = periodo, pch=trampa)) +
  # Ejes en el origen
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray80") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray80") +
  
  # Elipses de confianza (95%)
  stat_ellipse(geom = "polygon", alpha = 0.15, color = NA, level = 0.68) +
